...
<body class="bg-light p-4">
      ...
      <div id="contentWrapper">
        @if(old('contents'))
          @foreach(old('contents') as $i => $content)
            <div class="content mb-4 p-4 bg-white shadow-sm rounded">
              <h4>Content {{ $i + 1 }}</h4>
              <div class="mb-2">
                <input type="text" name="contents[{{ $i }}][content_title]" class="form-control form-control-sm mb-1"
                  value="{{ $content['content_title'] }}" placeholder="Content title">
                @error("contents.$i.content_title")
                  <div class="text-danger small">{{ $message }}</div>
                @enderror
              </div>

              <button type="button" class="addBlogBtn btn btn-sm btn-secondary mb-3">Add More Blog</button>

              <div class="blogs">
                @foreach($content['blogs'] as $j => $blog)
                  <div class="blog mb-3">
                    <h6>Blog {{ $j + 1 }}</h6>
                    <input type="text" name="contents[{{ $i }}][blogs][{{ $j }}][title]" class="form-control form-control-sm mb-1"
                      value="{{ $blog['title'] }}" placeholder="Blog title">
                    @error("contents.$i.blogs.$j.title")
                      <div class="text-danger small">{{ $message }}</div>
                    @enderror

                    <input type="text" name="contents[{{ $i }}][blogs][{{ $j }}][body]" class="form-control form-control-sm"
        ...
        @else
          <div class="content mb-4 p-4 bg-white shadow-sm rounded">
            <h4>Content 1</h4>
            <div class="mb-2">
              <input type="text" name="contents[0][content_title]" class="form-control form-control-sm mb-1" placeholder="Content title">
            </div>
            ...
            <div class="blogs">
              <div class="blog mb-3">
                <h6>Blog 1</h6>
                <input type="text" name="contents[0][blogs][0][title]" class="form-control form-control-sm mb-1" placeholder="Blog title">
                <input type="text" name="contents[0][blogs][0][body]" class="form-control form-control-sm" placeholder="Blog body">
              </div>
            </div>
          </div>
        @endif
  ...
  <script>
    $(document).ready(function () {
      let contentIndex = {{ old('contents') ? count(old('contents')) : 1 }};
      ...
      $('#addContentBtn').click(function () {
        ...
        contentTemplate.find('input[name$="[content_title]"]').val('');
        contentTemplate.find('input[name$="[content_title]"]').attr('name', `contents[${contentIndex}][content_title]`);
        ...
        firstBlog.find('input').each(function (index) {
          ...
          $(this).attr('name', `contents[${contentIndex}][blogs][0][${type}]`);
        });

        // Remove all blogs, append one clean
      ...
      $(document).on('click', '.addBlogBtn', function () {
        ...
        const blogTemplate = blogsWrapper.find('.blog:first').clone();
        blogTemplate.find('h6').text('Blog ' + (blogCount + 1));

        blogTemplate.find('input').each(function (index) {
          $(this).val('');
          const type = index === 0 ? 'title' : 'body';
          $(this).attr('name', `contents[${contentIdx}][blogs][${blogCount}][${type}]`);
        });
